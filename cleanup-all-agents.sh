...
if [ -z "$API_KEY" ]; then
    ...
    echo "1. Get system status: curl -H 'X-API-Key: \$API_KEY' https://product-engineer.fryansoftware.workers.dev/api/orchestrator/status"
    echo "2. Run cleanup: curl -X POST -H 'X-API-Key: \$API_KEY' https://product-engineer.fryansoftware.workers.dev/api/orchestrator/cleanup-inactive"
    echo ""
...
WORKER_URL="https://product-engineer.fryansoftware.workers.dev"
